...
const showRestaurants = position => {
  ...
  // adds marker of the user current position
  state.userMarker = markerView.marker(
    state.currentPosition,
    state.map
  );
...
const handleError = error => {
  ...
  if (error.code === 0) {
    ...
    alert(
      ...
      'User denied the request for Geolocation. Refresh the browser and allow Geolocation'
    );
      ...
      photos: [
        {
          ...
          photo_reference:
            // eslint-disable-next-line max-len
            'CmRaAAAAYG7ci6eFkJq765ie_fVjCQjNja2u2U9kmeEZ9ALDYe3-UiVV8m28ue_qDSt7KOV_BK-YzbvkrJbphtXMpPqNsOUW-l80J8Xo1AIu6blkrf-0cb8p6XardCc6fshJszOQEhA_aafnj1Z2undtu6A5YlWsGhS2XW9FF6aUlpSifMrTbDW5UmHpJg',
          width: 5220
        }
      ],
      ...
      photos: [
        {
          ...
          photo_reference:
            // eslint-disable-next-line max-len
            'CmRaAAAABAnYjprfHNrUx6e3mJNPY5sAs3Hkmw_p0mbfQQl4grii6RLYs_VneXPfhNgI5QnCah1bCoCOeus5syGr1ZMDe9G7js2pdYu_ZxV10ZUMQrtL8lTj79HI_FFuCel7WTYbEhA4yej4uUdoyi3e3CK95cGGGhQGtWC2aXYtczDjAueFzBVnyHRymA',
          width: 1181
        }
      ],
      ...
      photos: [
        {
          ...
          photo_reference:
            // eslint-disable-next-line max-len
            'CmRaAAAAU_t7wmXkRXx9mZ8JpLLRNR99foy3WyeQMeuTm9xXc43IhXf1QYFSK0V7ClfLt9zOosxOvo47JvJ8rorVR_a1jmi9EcFoufB4T_iUFckVM3kNJ4Oc1ZF18-IxmTaErW_OEhByU_MOZXwWFtJf-Q9kuebuGhSOVahXXrw3ywV8mHeQZXCB5OVFiw',
          width: 4026
        }
      ],
      ...
      photos: [
        {
          ...
          photo_reference:
            // eslint-disable-next-line max-len
            'CmRaAAAAZFYVoqYQ9e-zsOZYA62fPhIJpBymwna7jtn4A4m34CUIVSGnoumKU-eyKN2zguibKsXe0RuLdwJCk8Kcx5YXBIJJ2h62_9hp1agh08kHJ6RhFBSmaQhksJYNBGo1BFEBEhBalcwhiQtrCfuuuCP3R-gaGhTshI3-sa-kbrMo8-uZin9vzg3IsQ',
          width: 851
        }
      ],
      ...
      photos: [
        {
          ...
          photo_reference:
            // eslint-disable-next-line max-len
            'CmRaAAAA5_Io5RaEcVuHMtjQhBK9RHT4NGjaRoLDf6tzqMVBNjBI1hKRmEMOyRoubE4EMGVL-HfnPbNqeBCb_zzVz_VzNJxGHJix5F7VopehE5y1zsFJDCTFZ1gRISF2OFtq2D_4EhA4fPJ91Pw_xuO45a8NruXBGhRV2dg3EvITz8biCls3KdNouVuYcg',
          width: 4032
        }
      ],
...
const getNearbyRestaurants = position => {
  ...
  state.service = new window.google.maps.places.PlacesService(
    state.map
  );
  // eslint-disable-next-line no-use-before-define
...
const findRestaurants = () => {
  ...
  // google places API
  state.places = new window.google.maps.places.PlacesService(
    state.map
  );
...
const getNearbyRestaurantsCallback = (results, status) => {
  if (status === window.google.maps.places.PlacesServiceStatus.OK) {
    ...
    if (state.markers) {
      state.markers = markerView.clearMarkers(state.markers);
    }
    // eslint-disable-next-line max-len
    if (state.demoRestaurantsMarkers) {
      state.demoRestaurantsMarkers = markerView.clearMarkers(
        state.demoRestaurantsMarkers
      );
    }
    ...
    state.demoRestaurantsMarkers = markerView.setMarkers(
      state.map,
      state.demoRestaurants
    );
...
const findRestaurantsCallback = (results, status) => {
  if (status === window.google.maps.places.PlacesServiceStatus.OK) {
    ...
    // eslint-disable-next-line max-len
    if (state.demoRestaurantsMarkers) {
      markerView.clearMarkers(state.demoRestaurantsMarkers);
    }
...
const mapEventListeners = () => {
  ...
  state.demoRestaurantsMarkers.forEach(marker => {
    ...
    window.google.maps.event.addListener(marker, 'click', () => {
      // eslint-disable-next-line no-shadow
      const getRestaurantData = (data, id) =>
        data.filter(marker => marker.id === id);
      // const result = state.restaurants.getRestaurant(marker.id);
      // eslint-disable-next-line no-use-before-define
      // get marker details
      const result = getRestaurantData(
        state.demoRestaurants,
        marker.id
      );
  ...
  window.google.maps.event.addListener(
    ...
    event => {
      ...
      // sets a maker
      state.newMarkers.push(
        markerView.setMarker(state.map, restaurant)
      );
      ...
      state.newMarkers.forEach(marker => {
        ...
        window.google.maps.event.addListener(marker, 'click', () => {
          ...
          if (marker.isRestaurantNew) {
            ...
            const restaurantRatingStar = document.querySelectorAll(
              `#${elementStrings.restaurantRatingStar}`
            );
            ...
            if (restaurantForm) {
              ...
              restaurantForm.addEventListener('submit', event => {
                ...
                const id = document.querySelector(
                  `#${elementStrings.newRestaurantId}`
                ).value;
                const name = document.querySelector(
                  `#${elementStrings.newRestaurantName}`
                ).value;
                const address = document.querySelector(
                  `#${elementStrings.newRestaurantAddress}`
                ).value;
                const rating = document.querySelector(
                  `#${elementStrings.newRestaurantRating}`
                ).value;
                const locationLat = document.querySelector(
                  `#${elementStrings.newRestaurantLat}`
                ).value;
                const locationLng = document.querySelector(
                  `#${elementStrings.newRestaurantLng}`
                ).value;
                ...
                state.newRestaurants.push(
                  state.restaurants.addRestaurant(place)
                );
...
for (const option of document.querySelectorAll('.custom--option')) {
  ...
  option.addEventListener('click', function() {
    if (!this.classList.contains('selected')) {
      ...
      this.closest('.custom--select').querySelector(
        '.custom--select__trigger span'
      ).innerHTML = `${this.textContent} <span class="fa fa-star"></span>`;
    }
  });
}
...
const filterRestaurants = rating => {
  if (rating) {
    ...
    try {
      ...
      state.markers = markerView.setMarkers(
        state.map,
        state.filteredRestaurants
      );
